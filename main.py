...
def main():
    ...
    messages = [
        {"role": "system", "content": "You are JHATU, an elite Cybersecurity Assistant created by Amit (Team Dark). Keep responses sharp, precise, and professional."}
    ]

    while True:
        try:
            timestamp = datetime.now().strftime("%H:%M")
            prompt_ui = f"{Fore.RED}┌──[{Fore.WHITE}JHATU{Fore.RED}]─[{Fore.CYAN}{timestamp}{Fore.RED}]\n{Fore.RED}└──{Fore.GREEN}⚡{Fore.WHITE} "
            ...
            if code_blocks and any(w in cmd.lower() for w in ["write", "create", "generate"]):
